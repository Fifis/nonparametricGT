...
saveGIF({ # Takes roughly 30 seconds on Linux and a whopping 20 minutes on Windows because of the rug
  for (i in 1:101) {
...
}, movie.name = "34-density-estimators-the-film.gif", interval = 0.25, ani.width = 640, ani.height = 480, ani.dev = function(...) png(..., type="cairo"))

# Regression estimator with a slightly growing bandwidth
mu.grid <- seq(0, 27, length.out = 401)
saveGIF({ # Takes roughly 30 seconds
  for (i in 1:101) {
...
}, movie.name = "35-regression-estimators-the-film.gif", interval = 0.25, ani.width = 640, ani.height = 480, ani.dev = function(...) png(..., type="cairo"))
...
saveGIF({ # Takes roughly 30 seconds on Linux but 20 minutes on Windows!
  for (a in 1:360) {
    par(mar = c(2, 2, 2, 2))
...
}, movie.name = "36-kde-3d.gif", interval = 1 / 25, ani.width = 480, ani.height = 480, ani.dev = function(...) png(..., type="cairo"))
...
}, movie.name = "37-mu-3d.gif", interval = 1 / 25, ani.width = 480, ani.height = 480, ani.dev = function(...) png(..., type="cairo"))
