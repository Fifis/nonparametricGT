...
my.colours <- c("black", "blue", "red", "forestgreen", "darkorange1")
...
curve(kernelFun(x, kernel = "uniform", rescale = FALSE), -2, 2, ylim = c(0, 1.1), col = my.colours[1], lwd = 2, ylab = "Kernel", main = "Various kernels used in smoothing", bty = "n")
for (i in 2:5) curve(kernelFun(x, kernel = all.kernels[i], rescale = FALSE), -2, 2, add = TRUE, col = my.colours[i], lwd = 2)
legend("topright", legend = all.kernels, lwd = 1, col = my.colours, bty = "n")
...
curve(kernelFun(x, kernel = "uniform", rescale = TRUE), -3, 3, ylim = c(0, 1.1), col = my.colours[1], lwd = 2, ylab = "Rescaled kernel", bty = "n", main = "Rescaled kernels")
for (i in 2:5) curve(kernelFun(x, kernel = all.kernels[i], rescale = TRUE), -3, 3, add = TRUE, col = my.colours[i], lwd = 2)
legend("topright", legend = all.kernels, lwd = 1, col = my.colours, bty = "n")
dev.off()

for (k in all.kernels) print(integrate(function(x) kernelFun(x, kernel = k, rescale = FALSE), -Inf, Inf)) # Integrates to one.
for (k in all.kernels) print(integrate(function(x) kernelFun(x, kernel = k, rescale = TRUE), -Inf, Inf)) # Integrates to one.
for (k in all.kernels) print(integrate(function(x) x^2 * kernelFun(x, kernel = k, rescale = FALSE), -Inf, Inf)) # Depends on the shape of the kernel.
for (k in all.kernels) print(integrate(function(x) x^2 * kernelFun(x, kernel = k, rescale = TRUE), -Inf, Inf)) # Integrates to 1.
...
mtext("Convolutions of kernels", outer = TRUE) # Main title on top.
dev.off()
...
legend("topright", paste0(c("True density", "Optimal smthng (b=", "Under-smthng (b=", "Over-smthng (b="), c("", round(bws, 3)), c("", rep(")", 3))), bty = "n", col = c("black", "black", "red", "blue"), lty = c(2, 1, 1, 1), lwd = 2)
dev.off()
...
DCV.values <- lapply(all.kernels, function(k) sapply(bw.grid, function(b) DCV(X, bw = b, kernel = k)))
...
mtext("Density cross-validation", outer = TRUE)
dev.off()

# Or can find the exact minimum using the minimiser now that we know that this function has an optimum in this range
opt.bw.dcv <- sapply(all.kernels, function(k) optimise(function(b) DCV(X, bw = b, kernel = k), c(0.1, 5))$minimum)
...
for (i in 1:5) {
  ...
  rug(X)
  legend("topright", paste0("b_CV = ", round(opt.bw.dcv[i], 3)), bty = "n")
}
mtext("Density estimators with cross-validated bandwidths", outer = TRUE)
dev.off()
...
par(mar = c(4, 5, 4, 1))
plot(X, Y, xlab = "x", ylab = expression(hat(E)(Y ~ "|" ~ X == x)), bty = "n", main = paste0("Nadaraya-Watson estimator (b=", round(opt.bw.dcv["gaussian"], 3), ")"))
lines(Xgrid, mymuhat)
dev.off()

# Computing least-squares cross-validation function value for bandwidth 0.42
LSCV(X, Y, opt.bw.dcv["gaussian"], "gaussian")
LSCV(X, Y, opt.bw.dcv["gaussian"] * 1.5, "gaussian") # It is smaller!

# Cross-validating the bandwidth for the Nadaraya---Watson estimator using Least Squares
LSCV.values <- lapply(all.kernels, function(k) sapply(bw.grid, function(b) LSCV(X, Y, bw = b, kernel = k)))
...
opt.bw.lscv["gaussian"] <- optimise(function(b) LSCV(X, Y, bw = b, kernel = "gaussian"), c(0.1, 0.55))$minimum

fhat.opt <- sapply(all.kernels, function(k) kernelDensity(X, Xgrid, opt.bw.lscv[k], kernel = k))
...
title(main = paste0("bandwidth = ", round(opt.bw.lscv["gaussian"], 3)), font.main = 1)
...
lines(Xgrid, Xgrid^2, lty = 2)
title(main = paste0("bandwidth = ", round(opt.bw.lscv["uniform"], 3)), font.main = 1)

top.plot.title <- c("Density and regression estimates", paste0("n = ", n), paste0("Time(sec) = ", round(as.numeric(seconds.taken), 3)))
